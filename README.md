...
| --------------------------------------- | ---------- | ------------------ | ------------------------------------------------------------------------------------------- |
| [orchestrator](./plugins/orchestrator/) | 0.3.3      | SessionStart フック / `/route` | **コアプラグイン** — ルーティング表と運用規則（初回一括確認・以降は自動完遂）をコンテキスト注入し、メイン会話をオーケストレーターとして適切なプラグインへルーティング・調整させる |
...
| [postgres-dev](./plugins/postgres-dev/) | 0.1.0 | `/postgres-dev` | PostgreSQLのDDL/DMLを対象リポジトリのマイグレーション運用（Flyway等・命名規約・コード生成）に沿って作成・変更する（破壊的操作のみ実行前確認。質問は実行前確認の1回のみ） |
...
/backlog-manager 要件定義書から PBI を起票して
