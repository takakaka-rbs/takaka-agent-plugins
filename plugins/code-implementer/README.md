...
5. **テスト**: 単体（JUnit / Vitest）→ API（RestAssured）→ VRT（Storybook）→ E2E（Playwright）の順に、選択されたレベルを実行し、共通形式のレポートを出力する
...
| Agent | `agents/test-api.md` | RestAssured APIテスト（正常系・エラー条件・ビジネスルール）の作成・実行・レポート |
