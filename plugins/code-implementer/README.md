...
API仕様書（OpenSpec.yml）・画面仕様書を入力として、対象リポジトリのアプリケーションコードを「**技術調査 → 実装計画 → 実装 → テスト → 修正ループ → 実装レビュー**」のワークフローで自動実装するプラグインです。
...
`/code-implementer` スキルが以下を進行管理します。実装・テスト・レビューの実作業は11個のサブエージェントに委譲されます。
...
7. **実装レビュー**: 全テストレベル成功後、`dev-reviewer` が**実装コード・テストコード・テスト実行結果**の3点を仕様書・実装計画・リポジトリプロファイルに照らしてレビューする。must-fix は実装エージェント / テストエージェントに差し戻して再テスト・再レビューする（往復上限2回）。suggestion は差し戻さず最終報告に記録する
8. **完了報告**: 実装ファイル・テスト結果・レビュー結果・採用した仮定・未解決課題（仕様起因・環境起因を含む）を開示する
...
| Agent | `agents/dev-reviewer.md` | 全テスト成功後のレビューゲート（実装・テスト・結果のレビューと差し戻し指示） |
| 設計ドキュメント | `docs/architecture.md` | ワークフロー全体設計・エージェント責務・修正ループ終了条件 |
| 成果物テンプレート | `docs/artifact-templates.md` | 調査レポート・実装計画・テスト結果・修正計画・レビューレポートの共通形式 |
...
└── review-reports/review-attempt<N>.md
```
...
- `dev-reviewer` エージェントを追加: 全テストレベル成功後のレビューゲートとして、実装コード・テストコード・テスト実行結果をレビューし、must-fix を実装/テストエージェントに差し戻す（往復上限2回）。レビューレポートの成果物テンプレートを追加
- 全エージェントの frontmatter を修正: description を folded scalar 化（YAML パース失敗により description・ツール制限が無効化されていた問題の修正）、`model: claude-sonnet-4-6` を `model: sonnet` エイリアスに変更
